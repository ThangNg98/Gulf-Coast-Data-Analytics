...
def get_volunteer_session_history(volunteer_id):
    print('volunteer_id:', volunteer_id)
    query = """
SELECT event.event_name AS eventName, organization.org_name AS orgName, session.total_hours AS hours, session.session_date AS dateval 
FROM session
JOIN event ON event.event_id = session.event_id
LEFT JOIN organization ON organization.org_id = session.org_id
